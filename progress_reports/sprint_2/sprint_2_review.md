...
> Bản gốc của review này được viết cuối tuần 3 (05/08), khi lịch sprint còn đang chia không đều tuần
> (Sprint 2 = tuần 3). Sau khi thống nhất lại lịch 6 tuần / 2 tuần mỗi sprint (xem [README](../README.md)
> và PROPOSAL.md mục 8), tuần 4 chuyển vào Sprint 2. Mục 1–7 dưới đây giữ nguyên nội dung gốc (đúng thực tế
> tại thời điểm 05/08); mục 4b bổ sung tóm tắt công việc tuần 4 để review phản ánh đủ 2 tuần của sprint —
> chi tiết đầy đủ xem [week_4_summary.md](week_4/week_4_summary.md).
...
phương pháp luận — xem [Sprint 1 Review](../sprint_1/sprint_1_review.md) mục 6.1). Trạng thái kế thừa
...
## 4b. Bổ sung — Tuần 4 (06/08 – 12/08/2026)

Tuần 4 làm tiếp phần vốn thuộc phạm vi Sprint 3 (đã bắt đầu ngay từ cuối tuần 3, xem mục 4), thay vì việc
mới của Sprint 2 — bảng DoD ở mục 2 không đổi so với cuối tuần 3. Tóm tắt (chi tiết ở
[week_4_summary.md](week_4/week_4_summary.md)):

- **PSI (mục 5 bước 5, còn nợ)** — đã tính lần đầu ngày 10/08 (train→val 0.035, train→test 0.010,
  val→test 0.011), sau đó tính lại theo model đã tune (12/08): **train→val 0.033, train→test 0.011,
  val→test 0.010** — cả hai lần đều dưới ngưỡng 0.10 (ổn định); số liệu 12/08 là số liệu cuối cùng, khớp
  `reports/figures/psi_report.csv` hiện tại.
- **Sửa lỗi Expected Net Return** (mục 6, `int_rate` trước đó chỉ áp dụng 1 kỳ) — nay tính đủ dòng tiền
  theo kỳ hạn 3 năm. Phát sinh thêm 1 phát hiện phương pháp: chọn cutoff theo Expected Net Return tuyệt đối
  không còn hợp lý sau khi sửa (bị kéo lệch về phía duyệt gần hết hồ sơ) — đổi tiêu chí sang argmax uplift
  so với duyệt ngẫu nhiên. Cutoff đề xuất đổi từ 0.13 sang **0.17**.
- 2 business rules override không đổi so với kết luận cuối tuần 3 (dựa trên IV của biến thô, không phụ thuộc
  `pd_score` nên không bị ảnh hưởng khi model được tune lại — xem business_rules_policy.md mục 2).
- **Cắt 6 feature nhiễu + tune LightGBM** (chạy cuối ngày 12/08, nhưng commit bị trễ đến 14/08 — đầu tuần 5,
  xem daily log tuần 4 ngày 12/08): AUC/KS/Gini cải thiện lên **0.7023/0.2926/0.4047**, hệ số LR sai dấu giảm
  từ 9/40 xuống 4/34, cutoff đổi tiếp từ 0.17 sang **0.19**. Đây là kết quả model chính thức cuối Sprint 2.
  `pd_score` từng khoản vay đổi nhẹ theo model mới, kéo theo **segmentation cũng đổi nhẹ**: bad rate S1→S5
  đi từ 6.59%→38.58% (trước tuning) sang **6.61%→38.80%** (sau tuning, chi²=8012.81 thay vì 7892.78) — số
  liệu 6.61%→38.80% là số liệu cuối cùng, khớp `dashboards/segment_summary.csv` và `eda_risk_report.md` mục
  4 hiện tại.
- `business_rules_policy.md`, `final_recommendation.md`, `dashboards/*.csv` đã đồng bộ theo số liệu mới.
